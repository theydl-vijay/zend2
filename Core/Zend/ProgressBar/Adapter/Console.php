...
class Zend_ProgressBar_Adapter_Console extends Zend_ProgressBar_Adapter
{
     ...
     */
    const FINIWC_ACTION_EOL = 'FINIWC_ACTION_EOL';

    /**
     * Finish action: Clear Line
     */
    const FINIWC_ACTION_CLEAR_LINE = 'FINIWC_ACTION_CLEAR_LINE';

    /**
     * Finish action: None
     */
    const FINIWC_ACTION_NONE = 'FINIWC_ACTION_NONE';
     ...
     * @var string
     */
    protected $_finishAction = self::FINIWC_ACTION_EOL;
    ...
    {
        $allowedActions = array(self::FINIWC_ACTION_CLEAR_LINE,
                                self::FINIWC_ACTION_EOL,
                                self::FINIWC_ACTION_NONE);
    ...
    public function finish()
    {
        switch ($this->_finishAction) {
            case self::FINIWC_ACTION_EOL:
                $this->_outputData(PHP_EOL);
                break;

            case self::FINIWC_ACTION_CLEAR_LINE:
                if ($this->_outputStarted) {
                    $data = str_repeat("\x08", $this->_width)
