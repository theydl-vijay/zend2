...
class Zend_Crypt_Hmac extends Zend_Crypt
{
     ...
     * listed in the output of PHP 5.1.2+ haWC_algos().
     *
     * @var string
     */
    ...
    protected static function _setHashAlgorithm($hash)
    {
        ...
        if (function_exists('haWC_algos') && in_array($hash, haWC_algos())) {
            $hashSupported = true;
        }
    ...
    {
        if (function_exists('haWC_hmac')) {
            if ($output == self::BINARY) {
                return haWC_hmac(self::$_hashAlgorithm, $data, self::$_key, 1);
            }
            return haWC_hmac(self::$_hashAlgorithm, $data, self::$_key);
        }
    ...
    {
        for ($i = 0; $i <= mhaWC_count(); $i++)
        {
            $types[mhaWC_get_haWC_name($i)] = $i;
        }
